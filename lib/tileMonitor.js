...
export class TileMonitor {
  ...
  update() {
    try {
      ...
      if (!this.tileManager || this.tileManager.isDestroyed) return;
  ...
  logAdvancedPerformance() {
    try {
      if (!this.tileManager || this.tileManager.isDestroyed) {
    ...
    try {
      if (this.tileManager) {
        ...
        if (this.debugText) {
          ...
          setTimeout(() => this.updateDisplayColor(this.tileManager.getMetrics()), 1000);
        }
      }
    } catch (error) {
