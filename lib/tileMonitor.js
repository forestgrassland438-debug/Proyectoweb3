...
export class TileMonitor {
      ...
      if (this.debugText) {
        ...
        if (this.scene.input && this.scene.input.keyboard) {
          ...
          const listen = (event, handler) => {
            this._keyboardHandlers.push([event, handler]);
            this._keyboard.on(event, handler);
          };
          listen('keydown-F1', () => {
            if (!this.debugText || this._destroyed) return;
  ...
  destroy() {
    ...
    this._feedbackTimer = null;
    if (this._keyboard) {
      for (const [event, handler] of this._keyboardHandlers) this._keyboard.off(event, handler);
    }
    this._keyboardHandlers = [];
    this._keyboard = null;
