...
class TileManager {
  constructor(scene, meta, opts = {}) {
    ...
    /* Cuánto se espera antes de tirar de verdad un tile que ha salido de
       rango. Ver el periodo de gracia en updateVisible(). */
    this.graciaMs = (typeof opts.graciaDescargaMs === 'number')
      ? Math.max(0, opts.graciaDescargaMs) : 12000;
    ...
    this._ownedBitmaps = new Map();

    /* ═══ EL TIRÓN AL CAMINAR, Y DE DÓNDE SALE DE VERDAD ══════════════════
       Cada chunk es un PNG que, descomprimido, ocupa 2048 × 2048 × 4 = 16,8 MB.
       Con `loader.image()` el navegador DESCODIFICA ese PNG en el HILO
       PRINCIPAL, dentro del mismo tick en que termina la descarga, y acto
       seguido Phaser lo sube a la GPU. Son dos trabajos caros, seguidos, en un
       fotograma que tenía 16 ms de presupuesto: eso es la mini-pausa.

       Ya se había intentado arreglar bajando a UNA descarga a la vez. Reparte
       el problema, pero no lo quita: una sola descodificación de 2048² ya se
       come el fotograma. Y de paso hacía que los chunks llegaran tarde.

       Lo que se hace ahora es partir el trabajo en dos y sacar de en medio el
       más caro:

         · DESCODIFICAR → `fetch` + `createImageBitmap`, que el navegador
           resuelve en un HILO APARTE. El hilo principal no se entera.
         · SUBIR A LA GPU → se hace en bombear(), UNA por fotograma, que es lo
           único que de verdad tiene que pasar por el hilo principal.

       Y como descodificar ya no bloquea, se pueden volver a permitir varias
       descargas a la vez sin que se junten los tirones.

       Si el navegador no trae createImageBitmap se usa el camino de siempre. */
    this._soportaBitmap = (typeof createImageBitmap === 'function' &&
                           typeof fetch === 'function' &&
                           opts.usarImageBitmap !== false);
  ...
  init() {
    ...
    this.tilesByRC = Object.create(null);
    this.tilesByXY = Object.create(null);
  ...
  _sanitizePath(str) {
    ...
    // URL parsers normalize encoded dot segments, backslashes and whitespace.
    // Decode before validating so a manifest cannot escape its asset directory.
    let decoded;
    try { decoded = decodeURIComponent(str); } catch (e) { return ''; }
    if (
      decoded !== decoded.trim() || /[\x00-\x1f\x7f\\?#%]/.test(decoded) ||
  ...
  loadTile(row, col) {
    ...
    this.loadQueue.push({ row, col, key, url: info.url, w: info.w, h: info.h, x: info.x, y: info.y, _generation: this._loadGeneration });
  ...
  _processQueue() {
    ...
    if (this.scene.textures.exists(item.key)) {
      ...
      // Existing textures may belong to another manager; a cache hit is not ownership.
    ...
    this.currentLoads++;
    this.loading.add(item.key);
    this._pendingLoads.set(item.key, item);

    // Camino rápido: descodificar fuera del hilo principal.
    if (this._soportaBitmap) { this._descargarBitmap(item); return; }
    ...
    const onComplete = (fileKey) => {
      ...
      item._detach();
      this._finishLoad(item);
      if (!this._isCurrentLoad(item)) return;
      this._onTileLoaded(item, loader, onError);
    };

    // FIX #3: también escuchar errores de carga asíncrona
    const onError = (file) => {
      if (!file || file.key !== item.key) return;
    ...
    try {
      ...
      // Keep only this File, never reset the shared scene loader. A cancelled
      // File may finish decoding, but must not publish an abandoned texture.
      const files = loader.list && loader.list.entries;
      if (Array.isArray(files)) item._file = files.find(file => file.key === item.key);
  ...
  /**
   * Descarga y DESCODIFICA el chunk fuera del hilo principal.
   *
   * `createImageBitmap` hace el trabajo caro (descomprimir el PNG a píxeles) en
   * un hilo del navegador. Lo que vuelve ya está listo para subir a la GPU, y
   * esa subida se hace luego en bombear(), una por fotograma.
   *
   * La textura NO se registra aquí a propósito: registrarla es justo lo que
   * dispara la subida a la GPU, y hacerlo en el momento en que termina la
   * descarga es volver al problema — varias descargas pueden terminar en el
   * mismo tick.
   */
  _descargarBitmap(item) {
    const soltar = () => this._finishLoad(item);
    item._controller = typeof AbortController === 'function' ? new AbortController() : null;

    fetch(item.url, { credentials: 'omit', cache: 'force-cache', signal: item._controller ? item._controller.signal : undefined })
      .then((r) => {
        if (!r.ok) throw new Error('HTTP ' + r.status);
        return r.blob();
      })
      .then((blob) => createImageBitmap(blob, {
        // Que la premultiplicación y el espacio de color los haga la GPU, igual
        // que con una <img> normal: pedirlos aquí cambiaría el aspecto de los
        // tiles respecto al resto del juego.
        premultiplyAlpha: 'none',
        colorSpaceConversion: 'none',
        imageOrientation: 'none'
      }))
      .then((bmp) => {
        if (!this._isCurrentLoad(item) || !this.loading.has(item.key)) {
          // El tile se salió de rango mientras bajaba: se tira el mapa de bits
          // en vez de dejarlo en memoria.
          try { bmp.close(); } catch (e) {}
          soltar();
          return;
        }
        item._bitmap = bmp;
        soltar();                       // la descarga ha terminado de verdad
        this._listos.push(item);        // el sprite se crea en bombear()
        this._processQueue();
      })
      .catch((e) => {
        soltar();
        ...
        console.warn(`❌ Error de carga para tile ${item.key}: ${item.url}`,
                     e && e.message);
        this.loading.delete(item.key);
        /* Sin createImageBitmap para este archivo (un blob corrupto, un
           navegador raro): se apaga el camino rápido para los siguientes y se
           vuelve al cargador de Phaser, que es más lento pero nunca falla. */
        if (e && /createImageBitmap|decode/i.test(String(e.message || ''))) {
          this._soportaBitmap = false;
        }
        this._processQueue();
      });
  }
  ...
  _isCurrentLoad(item) {
    return !this._destroyed && !item._cancelled && item._generation === this._loadGeneration;
  }

  _finishLoad(item) {
    if (this._pendingLoads.get(item.key) === item) this._pendingLoads.delete(item.key);
    if (item._httpLoad && item._generation === this._loadGeneration) {
  ...
  _cancelLoad(item) {
    ...
    if (item._detach) item._detach();
    if (item._controller) item._controller.abort();
    if (item._file) item._file.addToCache = function () {};
    this._finishLoad(item);
    this.loading.delete(item.key);
  }

  _cancelPendingLoads() {
    for (const item of this._pendingLoads.values()) this._cancelLoad(item);
    this._pendingLoads.clear();
    this._loadGeneration++;
    for (const item of this._listos) {
      if (item._bitmap) { try { item._bitmap.close(); } catch (e) {} item._bitmap = null; }
    }
    this._listos = [];
    this._paraDescarga = [];
    this.loadQueue = [];
  ...
  _createTileSprite(item) {
    ...
    /* AQUÍ, Y SOLO AQUÍ, SE SUBE A LA GPU.

       bombear() llama a este método como mucho `creaPorFrame` veces por
       fotograma (1 por defecto), así que por muchas descargas que terminen a la
       vez nunca se juntan dos subidas de 16 MB en el mismo fotograma. Es la
       otra mitad del arreglo del tirón. */
    if (item._bitmap) {
      try {
        if (!this.scene.textures.exists(item.key)) {
          this.scene.textures.addImage(item.key, item._bitmap);
          this._ownedBitmaps.set(item.key, item._bitmap);
          this._texturasPropias.add(item.key);
        } else {
          // Ya estaba (otro camino se adelantó): el mapa de bits sobra.
          try { item._bitmap.close(); } catch (e) {}
        }
      } catch (e) {
        console.warn(`⚠️ No se pudo registrar la textura ${item.key}:`, e && e.message);
        try { item._bitmap.close(); } catch (e2) {}
        item._bitmap = null;
        this.loading.delete(item.key);
        this._processQueue();
        return;
      }
      /* La referencia se suelta pero NO se cierra: Phaser se queda con ella
         dentro de la TextureSource y la vuelve a usar si el navegador pierde y
         recupera el contexto WebGL. Cerrarla dejaría el terreno en blanco tras
         una recuperación de contexto. */
      item._bitmap = null;
    }
  ...
  bombear() {
    ...
    if (creados === 0) {
      ...
      while (borrados < this.borraPorFrame && this._paraDescarga.length) {
        ...
        const info = this.loaded.get(this.tileKey(t.row, t.col));
        /* Puede haber vuelto a entrar en rango mientras esperaba en la cola:
           updateVisible() le pone `fueraDesde` a 0 en cuanto vuelve, y entonces
           tirarlo sería exactamente el ciclo destruir→recrear que se quiere
           evitar. */
        if (info && info.fueraDesde) this.unloadTile(t.row, t.col);
        borrados++;
      }
    }
  ...
  _liberarTextura(key) {
    ...
    try {
      ...
      const bitmap = this._ownedBitmaps.get(key);
      if (bitmap) { bitmap.close(); this._ownedBitmaps.delete(key); }
    } catch (e) {
  ...
  unloadTile(row, col) {
    ...
    if (pending) this._cancelLoad(pending);
    ...
    for (let i = this._listos.length - 1; i >= 0; i--) {
      if (this._listos[i].key === key) {
        // Un mapa de bits descodificado y sin usar son 16 MB en memoria: se
        // cierra a mano en vez de esperar al recolector de basura.
        const pend = this._listos[i];
        if (pend._bitmap) { try { pend._bitmap.close(); } catch (e) {} pend._bitmap = null; }
  ...
  updateVisible(camera) {
    ...
    /* ═══ PERIODO DE GRACIA ANTES DE TIRAR UN TILE ═══════════════════════
       EL TIRÓN DE "ANDAR MUCHO POR EL MAPA", LA PARTE QUE FALTABA.

       Subir un chunk a la GPU cuesta unos 21 ms medidos (2048² = 16,8 MB). Con
       la histéresis de 400 px, pasear de un lado a otro de una frontera —que es
       lo que se hace jugando— tiraba el tile al cruzar y lo volvía a subir al
       volver: un fotograma perdido en cada pasada, una y otra vez.

       Ahora, cuando un tile sale del rango, se apunta la HORA en que salió y no
       se toca hasta que pasen unos segundos. Si el jugador vuelve antes —que es
       el caso normal— no ha pasado nada: ni descarga, ni descodificación, ni
       subida. Si de verdad se ha ido, la memoria se recupera igual, solo que
       unos segundos más tarde.

       El coste es como mucho un par de tiles extra vivos durante esos segundos.
       El ahorro es el tirón entero cada vez que se cruza una frontera. */
    const ahoraTiles = Date.now();
    const toUnload = [];
    for (const [key, info] of this.loaded.entries()) {
      const fuera = (info.row < uTop || info.row > uBottom ||
                     info.col < uLeft || info.col > uRight);
      if (!fuera) { info.fueraDesde = 0; continue; }
      if (!info.fueraDesde) { info.fueraDesde = ahoraTiles; continue; }
      if (ahoraTiles - info.fueraDesde < this.graciaMs) continue;
      toUnload.push({ row: info.row, col: info.col, key: key });
    }
    /* Se APUNTAN, no se destruyen aquí: destruir diez tiles de 16 MB en el
       ...
       Sin repetir: updateVisible() corre cada 250 ms y, mientras bombear()
       estuviera ocupado creando, el mismo tile se apuntaba una y otra vez. */
    for (let i = 0; i < toUnload.length; i++) {
      const t = toUnload[i];
      if (this._paraDescarga.some((x) => x.row === t.row && x.col === t.col)) continue;
      this._paraDescarga.push(t);
    }
  ...
  /**
   * Igual que setMargin, pero en PÍXELES.
   *
   * FIX "LA DISTANCIA DE VISIÓN NO CAMBIA EL TERRENO": en cuanto un gestor se
   * crea con `marginPx` (GameScene lo hace), updateVisible() usa ESE valor y
   * `margin` deja de leerse. El panel de gráficos solo llamaba a setMargin(),
   * así que la barra movía un número que nadie miraba: el anillo de terreno se
   * quedaba en su valor de fábrica, subieras o bajaras la distancia.
   */
  setMarginPx(px) {
    const n = Math.max(0, Math.round(Number(px) || 0));
    if (n === this.marginPx) return false;
    this.marginPx = n;
    if (this.scene && this.scene.cameras && this.scene.cameras.main) {
      this.updateVisible(this.scene.cameras.main);
    }
  ...
  setLOD(name) {
    ...
    // Release using the OLD keys before switching LOD. In-flight callbacks
    // are invalidated even when the user switches hd → low → hd quickly.
  ...
  /**
   * LIMPIEZA DE EMERGENCIA: suelta toda la memoria pesada pero NO mata al
   * gestor. Si después se le vuelve a llamar a `updateVisible()`, recarga lo
   * que haga falta.
   *
   * ═══ ESTE MÉTODO NO EXISTÍA, Y SE LE LLAMABA DESDE TRES SITIOS ═══
   *
   * GameScene lo invoca al apagar la escena y al cambiar de escena, siempre
   * así:
   *
   ...
   *     }
   *
   * Como el método no existía, la guarda daba false y la llamada NO HACÍA
   * NADA — pero el `console.log` de al lado seguía diciendo "limpiando
   * TileManagers", así que en la consola parecía que se estaba limpiando. Es
   * el peor tipo de fallo: silencioso y con un mensaje que dice lo contrario.
   *
   * La diferencia con `destroy()` es que aquél deja el gestor inservible
   * (`_destroyed = true`), y estos tres sitios lo llaman en momentos en los
   * que la escena PUEDE seguir viva — por ejemplo el envoltorio de
   * `sceneManager.start`, que corre antes de arrancar cualquier escena. Por
   * eso aquí no se marca `_destroyed`: se sueltan los megas y se deja el
   * gestor listo para volver a cargar.
   *
   * @returns {{tiles:number, texturas:number}} lo que se ha soltado
   */
  emergencyCleanup() {
    if (this._destroyed) return { tiles: 0, texturas: 0 };
    this._cancelPendingLoads();

    const antesTex = this._texturasPropias ? this._texturasPropias.size : 0;
    const claves = Array.from(this.loaded.keys());
    let tiles = 0;

    claves.forEach((key) => {
      const data = this.loaded.get(key);
      if (!data) return;
      // `unloadTile` quiere fila y columna; se sacan de los datos del tile.
      if (typeof data.row === 'number' && typeof data.col === 'number') {
        this.unloadTile(data.row, data.col);
      } else {
        // Sin fila/columna a mano se hace lo mismo pero a pelo.
        try { if (data.sprite && data.sprite.destroy) data.sprite.destroy(); } catch (e) {}
        this.loaded.delete(key);
      }
      /* Y la textura SIEMPRE, aunque `liberarTexturas` esté apagado.
         Esa opción existe para cambiar memoria por velocidad mientras se
         juega —conservar el tile por si el jugador vuelve—, pero esto es una
         limpieza de emergencia: aquí lo que se quiere es soltar los megas.
         `_liberarTextura` no hace nada si la textura ya no estaba. */
      this._liberarTextura(key);
      tiles++;
    });
    // Textures can be loaded but still awaiting a sprite, or retained when
    // liberarTexturas=false. They also belong to this manager.
    for (const key of Array.from(this._texturasPropias)) this._liberarTextura(key);

    /* Los mapas de bits ya descodificados que aún no llegaron a sprite: 16 MB
       cada uno, y el recolector de basura no los suelta solo. */
    for (const item of this._listos) {
      if (item && item._bitmap) { try { item._bitmap.close(); } catch (e) {} item._bitmap = null; }
    }
    this._listos = [];
    this._paraDescarga = [];

    // Y la cola: no tiene sentido seguir bajando tiles de una escena que se va.
    this.loadQueue = [];
    this.loading.clear();
    this.currentLoads = 0;

    const texturas = antesTex - (this._texturasPropias ? this._texturasPropias.size : 0);
    if (tiles || texturas) {
      console.log(`🧹 TileManager (emergencia): ${tiles} tile(s) y ${texturas} textura(s) ` +
                  `liberadas (~${Math.round(texturas * 16.8)} MB). El gestor sigue vivo.`);
    }
    return { tiles, texturas };
  }
  ...
  destroy() {
    ...
    // Mapas de bits descodificados que nunca llegaron a sprite: 16 MB cada uno.
    for (const item of this._listos) {
      if (item && item._bitmap) { try { item._bitmap.close(); } catch (e) {} item._bitmap = null; }
    }
    this._listos       = [];
