...
class TileManager {
  ...
  _createTileSprite(item) {
    if (this._destroyed) {
      ...
      this.loading.delete(item.key);
      // currentLoads solo se incrementó en cargas HTTP reales (no en cache hits),
      // así que decrementar solo si venía de una carga real
      if (this.currentLoads > 0) this.currentLoads--;
      return;
    }
