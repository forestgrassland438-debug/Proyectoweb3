...
*/
    ...
    */
    ...
    // Store Button — opens external store in new tab
...
if (dog.shadowContainer) {
  ...
  dog.shadowContainer.setDepth(dogFeetY - 1);
    ...
    if (blockedX || blockedY) {
      // Slide animation based on movement speed at collision
      // dx/dy may not be in scope here (mouse movement path), so derive from frame delta
      const _colDx = nextX - prevX;
      const _colDy = nextY - prevY;
      const speed = Math.hypot(_colDx, _colDy);
      if (speed > 2) {
        // Still moving fast — play walk/run in current direction
        const dir = blockedX ? (_colDy > 0 ? 'down' : 'up') : (_colDx > 0 ? 'right' : 'left');
        const animKey = `walk_${dir}`;
        // anims.isPlaying is a boolean property in Phaser 3, not a function.
        // Use currentAnim?.key to avoid replaying the same animation.
