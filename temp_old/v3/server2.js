...
io.on("connection", (socket) => {
  ...
  function assertCropOwner(userId, emitEvent) {
    if (socket.authenticatedPlayer && socket.authenticatedPlayer !== userId) {
      socket.emit(emitEvent, { error: 'No autorizado: userId no coincide con el jugador autenticado' });
      return false;
    }
    return true;
  }
  ...
  socket.on('plantSeed', async (data) => {
    try {
      ...
      if (!assertCropOwner(userId, 'plantError')) return;
  ...
  socket.on('harvestCrop', async (data) => {
    try {
      const { userId, plotId } = data;
      if (!assertCropOwner(userId, 'harvestError')) return;
  ...
  socket.on('cutCrop', async (data) => {
    try {
      const { userId, plotId } = data;
      if (!assertCropOwner(userId, 'cutError')) return;
...
app.get('/api/marketplace/listings',
  ...
  async (req, res) => {
    try {
      ...
      console.error('Error en marketplace:', err);
      return res.status(500).json({ error: 'Error interno' });
    }
  }
);
...
const STAT_DEFAULTS_MAP  = { vida: 100, agua: 100, comida: 100, oro: 2000, plata: 2000 };
