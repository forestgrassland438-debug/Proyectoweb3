...
updateOtherPlayer(playerInfo) {
  ...
  if (player._chatText) {
    ...
    player._chatText.setPosition(playerInfo.x, playerInfo.y - sprH*0.5 - 28);
  }
  if (player._chatTypingText) {
    const sprH = player.sprite.displayHeight || 64;
    player._chatTypingText.setPosition(playerInfo.x, playerInfo.y - sprH*0.5 - 22);
  }
...
removeOtherPlayer(playerId) {
  ...
  _positionLocalBubble(bubble) {
    ...
    bubble.style.top  = (sy - sprH * 0.5 - 12) + 'px';
  }
  ...
  _showRemoteChatBubble(playerId, text) {
    ...
    if (op._chatText) { op._chatText.destroy(); op._chatText = null; }
    if (op._chatTypingText) { op._chatTypingText.destroy(); op._chatTypingText = null; }
    clearTimeout(op._chatTimer);
    const display = text.length > 40 ? text.slice(0,40) + '…' : text;
    const sprH = op.sprite.displayHeight || 64;
    op._chatText = this.add.text(op.sprite.x, op.sprite.y - sprH*0.5 - 28, display, {
      fontFamily: '"PressStart2P"', fontSize: '8px', color: '#d0eaff',
      backgroundColor: 'rgba(10,20,50,0.88)', padding:{x:6,y:4},
      wordWrap:{width:160}, stroke:'#001030', strokeThickness:2, resolution:4,
    }).setOrigin(0.5,1).setDepth(op.sprite.depth+10);
    op._chatTimer = setTimeout(() => { if(op._chatText){op._chatText.destroy();op._chatText=null;} }, 4000);
  }
  _showRemoteTypingBubble(playerId, show) {
    ...
    if (!show) return;
    const sprH = op.sprite.displayHeight || 64;
    op._chatTypingText = this.add.text(op.sprite.x, op.sprite.y - sprH*0.5 - 22, '...', {
      fontFamily: '"PressStart2P"', fontSize: '10px', color: '#40a0ff',
      backgroundColor: 'rgba(10,20,50,0.80)', padding:{x:6,y:3}, resolution:4,
    }).setOrigin(0.5,1).setDepth(op.sprite.depth+10);
    op._typingHideTimer = setTimeout(() => { if(op._chatTypingText){op._chatTypingText.destroy();op._chatTypingText=null;} }, 3000);
  }
