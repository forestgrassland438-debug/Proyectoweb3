...
const battleHubOverlay = document.getElementById("battleHubOverlay");
const battleHubCloseBtn = document.getElementById("battleHubCloseBtn");
const battleHubAdventureBtn = document.getElementById("battleHubAdventureBtn");
const battleHubPvpBtn = document.getElementById("battleHubPvpBtn");

// Mostrar menú
function showBattleHub() {
  battleHubOverlay.classList.remove("hidden");
}

// Ocultar menú
function hideBattleHub() {
  battleHubOverlay.classList.add("hidden");
}

// Eventos de botones
battleHubCloseBtn.addEventListener("click", hideBattleHub);

battleHubAdventureBtn.addEventListener("click", () => {
  hideBattleHub();
  window.dispatchEvent(new CustomEvent("battlehub:adventure"));
});

battleHubPvpBtn.addEventListener("click", () => {
  hideBattleHub();
  window.dispatchEvent(new CustomEvent("battlehub:pvp"));
});

// Cerrar al hacer click fuera del panel
battleHubOverlay.addEventListener("click", (e) => {
  if (e.target === battleHubOverlay) {
    hideBattleHub();
  }
});
...
if (dog.shadowContainer) {
  ...
  dog.shadowContainer.setDepth(dogFeetY - 1);
    ...
    this.collisionRectangles2.forEach(rect1 => {
      if (Phaser.Geom.Intersects.RectangleToRectangle(this.playerRect, rect1)) {
        console.log("¡Colisión detectada!");
